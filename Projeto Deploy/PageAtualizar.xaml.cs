...
{
    public sealed partial class PageAtualizar : Page
    {
            ...
            BrowserButton.IsEnabled = false;
            {
               Progress<DeploymentProgress> progress = new Progress<DeploymentProgress>(inatallProgress);
                if (dependency != null && dependency.Count > 0)
                {
                    try
                    {
                        var result = await pkgPackage.UpdatePackageAsync(new Uri(packageInContext.Path), dependency, DeploymentOptions.ForceApplicationShutdown).AsTask(progress);
                    }
                    catch (Exception X)
                    {
                ...
                else
                {
                    try
                    {
                        var result = await pkgPackage.UpdatePackageAsync(new Uri(packageInContext.Path), null, DeploymentOptions.ForceApplicationShutdown).AsTask(progress);
                    }
                    catch (Exception x)
                    {
                        BorderOption.Visibility = Visibility.Visible;
        ...
        {
            var loader = new Windows.ApplicationModel.Resources.ResourceLoader();
            var text4 = loader.GetString("text4");
            var text3 = loader.GetString("text3");
            var text1 = loader.GetString("text1");
            double installPercentage = inatallProgress.percentage;
            string percentageAsString = String.Format($"{installPercentage}%");
            ProgressValue.Text = text4 + percentageAsString;
